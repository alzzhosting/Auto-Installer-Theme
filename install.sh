...
check_token() {
  ...
  if [ "$USER_TOKEN" = "kenzdev" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Skyzopedia${NC}"
    echo -e "${YELLOW}TELEGRAM : @kenzgabut${NC}"
    echo -e "${YELLOW}WHATSAPP : +6285934686607${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© KenzDeveloper${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/alzzhosting/Auto-Installer-Theme/raw/main/stellar.zip")        
        break
        ;;
      2)
        THEME_URL=$(echo -e "https://github.com/alzzhosting/Auto-Installer-Theme/raw/main/billing.zip")
        break
        ;;
      3)
...
if [ "$SELECT_THEME" -eq 1 ]; then
  ...
  yarn build:production
  php artisan view:clear
  sudo rm /root/darkenate-2.0.2.zip
  sudo rm -rf /root/pterodactyl
...
# Unduh file tema
wget -O /root/stellar.zip https://github.com/kenz-developer/auto-instal/raw/main/stellar.zip
...
while true; do
  ...
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : KenzDev      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : KenzDev  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by KenzDev  ${NC}"
